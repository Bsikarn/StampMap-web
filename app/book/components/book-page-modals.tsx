  ...
  return (
    <>
      ...
      <Dialog open={showChangeBook} onOpenChange={setShowChangeBook}>
        <DialogContent className={cn("mx-auto max-w-sm overflow-hidden rounded-[28px] p-0", modalClassName)}>
          ...
          <div className="max-h-72 space-y-2 overflow-y-auto px-4 py-4">
            {stampBooks.map((book) => (
              ...
              >
                {/* Book icon */}
                <div
                  className="flex h-12 w-12 shrink-0 items-center justify-center rounded-[14px] bg-gradient-to-br from-brand/12 to-accent-purple/8 border border-brand/15"
                >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1.5 mb-1">
                    ...
                    <span
                      className="flex items-center gap-0.5 rounded-full px-1.5 py-0.5 text-[9px] font-black bg-surface-subtle text-ink-muted"
                    >
          ...
          <div className="space-y-4 px-6 py-5">
            <div>
              ...
              <Select value={selectedCountry} onValueChange={(val) => setSelectedCountry(val ?? "")}>
                <SelectTrigger
                  className="rounded-[14px] text-sm font-medium w-full justify-center [&>span]:mx-auto bg-surface-subtle border border-brand/12 text-ink"
                >
      ...
      <Dialog open={showNfcModal} onOpenChange={setShowNfcModal}>
        <DialogContent className={cn("mx-auto max-w-xs overflow-hidden rounded-[28px] p-0", modalClassName)}>
          <div className={cn("px-6 pb-4 pt-6 text-center", headerClassName)}>
            <DialogHeader>
          ...
          <div className="flex flex-col items-center px-6 py-8 gap-5">
            ...
            <div className="relative">
              ...
              {/* Phone icon */}
              <div
                className="relative flex h-20 w-20 items-center justify-center rounded-[22px] animate-stamp-pulse gradient-jeju shadow-[inset_0_4px_8px_rgba(255,255,255,0.30),0_12px_32px_rgba(59,108,244,0.45)]"
              >
      ...
      <Dialog open={showSuccessModal} onOpenChange={setShowSuccessModal}>
        <DialogContent className={cn("mx-auto max-w-xs overflow-hidden rounded-[28px] p-0", modalClassName)}>
          ...
          <div className="flex flex-col items-center px-6 py-6 gap-4 animate-stamp-fade-in">
            {/* Success icon */}
            <div
              className="flex h-20 w-20 items-center justify-center rounded-[22px] bg-gradient-to-br from-success/12 to-success/6 border-[1.5px] border-success/25 shadow-[0_8px_24px_rgba(34,197,94,0.20)]"
            >
